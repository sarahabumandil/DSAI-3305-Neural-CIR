...
if __name__ == "__main__":
    ...
    numpy.set_printoptions(formatter={"float": lambda x: ("%4.1f" % x)})
    print()
    print("Term-document matrix A:")
    print(A)
    print()

    # Compute the SVD and show it.
    U, S, V = numpy.linalg.svd(A)
    S = numpy.diag(S)
    V = V[0:4,:]
    print("U, S, V from the SVD of A:")
    print(U)
    print()
    print(S)
    print()
    ...
    Vk = V[0:k, : ]
    Ak = Uk.dot(Sk).dot(Vk)
    print("Best rank-%d approximation:" % k)
    print(Ak)
    print()

    # Check that U.S.V is indeed A.
    # print("Compute U.S.V and check that it's A again:")
    # print(U.dot(S).dot(V))
    # print()

    # Check that Ut.U is the identity matrix.
